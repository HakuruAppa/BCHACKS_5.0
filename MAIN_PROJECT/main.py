...
USER_INFO_FILE_PATH = 'reg_result.txt' #extracting user info 

#Arrays
commands = [
    ('/start', 'Initializes the bot.'), ('/help', 'Lists all known commands.'), ('/status', 'Provides a prediction of how likely you are to be sick.'), 
    ('/register', 'Initiates registration process.'), ('/regCancel', 'Cancels the registration process'), ('/healthInfo', 'Records your current health data and provides a prediction of your status.'),
    ('/infoCancel', 'Terminates the health information process.'), ('/report', 'Self report confirmed illness without medical verification.')
...
def prediction():
    ...
    with open('IllnessPrediction.py', 'r') as f:
        exec(f.read())
        
    illness='99'
    illnessResultFile = open('illness_result.txt', 'r')
    illnessPull = illnessResultFile.readline()
    print(illnessPull)
    if illnessPull != "":
        illness = illnessPull
    illnessResultFile.close()
    return str(illness[0:4])
...
async def report_choice(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.data=="2":
        ...
        with open('userchats.txt', 'r') as userChatsFile:
            ...
            for i in temp:
                if i!=str(chat_id):
                    await context.bot.send_message(chat_id=i, text=announcement)
            userChatsFile.close()
...
async def status_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg=''
    illness=prediction()
    intIll = int(float(illness)*1000)
    print(intIll)
    if illness == '99':
        msg= 'Please initialize your info with /healthInfo!'
    elif intIll<100:
        msg= "You are highly unlikely to be ill."
    elif intIll<400 & intIll>100:
        msg= "You are not likely to be ill."
    elif intIll<900:
        msg= "There is a small chance you may be ill."
    else:
        msg= "You are highly likely to be ill. Please seek medical assistance or a SMART Test kit immediately."
...
async def get_id_number(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_data_array.append({'Name': user_name})
    user_data_array.append({'ID_Number': user_id})#stores username and id into user_data array
...
async def healthInfo_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text('We will begin your in-depth analysis now.') #actual bullshit
...
async def get_heart(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_heart == '1' or user_heart == '0':
        ...
        user_info_to_file(USER_INFO_FILE_PATH, user_info_array)
        await asyncio.sleep(3)
        await update.message.reply_text('Your records are now complete. Please use /status to predict your health status.')
        return ConversationHandler.END
    else:
        await update.message.reply_text('Re-enter a valid answer.')
        return HTRY_HEART

async def infoCancel_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
if __name__ == '__main__':
    ...
    information_handler = ConversationHandler(
        entry_points = [CommandHandler('healthInfo', healthInfo_command)],
        states ={
        ...
        fallbacks = [CommandHandler('infoCancel', infoCancel_command)]
    )
