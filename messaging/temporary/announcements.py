...
async def report_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    keyboard = [[InlineKeyboardButton("Yes", callback_data=str(r_1)),
            InlineKeyboardButton("No", callback_data=str(r_2))]]
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.message.reply_text("Would you like to self report illness?", reply_markup=reply_markup)
    return start_report


async def r_one(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
...
async def r_two(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Show new choice of buttons"""
    query = update.callback_query
    await query.answer()
    await update.message.reply_text('Please use the /register command to link Eunai with your data')
...
if __name__ == '__main__':
    ...
    states={
        start_report: [
            CallbackQueryHandler(r_one, pattern="^"+str(r_1)+"$"),
            CallbackQueryHandler(r_two),
            ]
       
    },
    fallbacks=[CommandHandler("report", report_command)],
    )
